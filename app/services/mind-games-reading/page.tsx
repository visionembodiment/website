...
export default function MindGamesReadingPage() {
  const { sections, hero, whatIs, process, testimonials, pricing, faq, cta } = mindGamesReadingContent;

  const pricingSection = (
    <LazySection key="pricing" animation="fade" delay={100} className={cn("bg-gradient-to-b from-vision-dark-purple to-vision-dark-purple-2 text-white", designSystem.spacing.section.full)}>
      <div className={designSystem.layout.container}>
        <div className={cn(designSystem.layout.maxWidth["3xl"], designSystem.spacing.margin.horizontal.auto, designSystem.layout.textAlign.center)}>
          ...
          <div className={cn("bg-white/10 backdrop-blur", designSystem.rounded.xl, designSystem.spacing.padding.lg, designSystem.spacing.margin.bottom.lg)}>
            ...
            <p className="text-purple-100">{pricing.description}</p>
          </div>
  ...
  return (
    <ServiceDetailTemplate
      ...
      faq={faq}
      cta={cta}
      customSections={{ pricing: pricingSection }}
    />
  );
}
